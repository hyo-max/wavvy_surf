...
def spot_list(request):
    ...
    south = ForcastData.objects.filter(category='2').order_by('title') #남해안
    east = ForcastData.objects.filter(category='1').order_by('title')  #동해안
...
class SchoolDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):   
        ...
        waveHeight = []
        for w in weathers:
            waveHeight.append(w.waveHeight)
